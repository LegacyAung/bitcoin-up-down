...
class MarketManager:
    ...
    async def market_disconnector(self):
        ...
        for ts in list(state.active_clob_streams):
            try:
                await self.data_manager.handle_disconnect_clob_wss(ts,'market')
            except Exception as e:
                print(f"⚠️ CLOB Market ({ts}) disconnect error: {e}")

        if state.user_channel_ts:
            try:
                await self.data_manager.handle_disconnect_clob_wss(state.user_channel_ts, 'user')
            except Exception as e:
...
#----------------------------------Monitors-------------------------------------#
    ...
    async def _dynamic_clob_wss_monitor(self):
        if state.res_count == 0 :
            new_timestamps = {
                'current': self._get_curr_res_ts(),
                'next': self._get_next_res_ts()
            }
        ...
        while self.is_running:
            try: 
                state.delta_sec = self._get_delta_sec()
                if state.delta_sec >= 900:
                    old_next = state.rolling_timestamps['next']
                    state.rolling_timestamps['current'] = old_next
                    state.rolling_timestamps['next'] = self._get_next_res_ts()
                    state.res_count += 1
                    print(f"✅ Timestamps Rotated: {state.rolling_timestamps}")
    ...
    async def _monitor_binance_rest_15m_persistance(self):
        while self.is_running:
            try:
                if state.delta_sec >= 899:
                    await self.data_manager.handle_persistant_15m_binance_rest(state.delta_sec)
                    await asyncio.sleep(5)
